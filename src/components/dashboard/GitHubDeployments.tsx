...
import {
  Rocket, Plus, RefreshCw, Trash2, StopCircle, Terminal,
...
const GitHubDeployments = () => {
  ...
  const statusBadge = (status?: string) => {
    ...
    if (s.includes("running"))
      return <Badge className="bg-success/10 text-success border-success/20"><CheckCircle2 className="w-3 h-3 mr-1" />Running</Badge>;
    if (s.includes("building") || s.includes("starting") || s.includes("restarting"))
      return <Badge className="bg-warning/10 text-warning border-warning/20"><Clock className="w-3 h-3 mr-1 animate-spin" />Building</Badge>;
      ...
      {view === "list" && (
        <div className="flex items-center justify-end gap-2">
          ...
          <Button size="sm" variant="accent" onClick={() => setView("create")}>
            <Plus className="w-4 h-4 mr-1" /> Deploy from GitHub
          </Button>
        </div>
      )}

      {view !== "list" && (
        <Button variant="ghost" size="sm" onClick={() => { setView("list"); setSelectedApp(null); }}>
            ...
            <Card className="border-dashed">
              <CardContent className="py-16 text-center">
                ...
                <Button variant="accent" onClick={() => setView("create")}>
                  <Plus className="w-4 h-4 mr-2" /> Deploy from GitHub
                </Button>
              </CardContent>
            </Card>
          ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button variant="accent" onClick={handleCreateApp} disabled={actionLoading}>
            ...
            <CardContent>
              {selectedApp.fqdn && (
                <div className="flex items-center gap-2 mb-4 p-3 bg-muted/50 rounded-lg">
                  ...
                  <a href={String(selectedApp.fqdn)} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline font-medium">
                    {String(selectedApp.fqdn)}
                  </a>
                </div>
              )}
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="accent" onClick={() => handleDeploy(selectedApp.uuid)} disabled={actionLoading}>
                ...
                <Button size="sm" variant="outline" onClick={() => setStopTarget(selectedApp)} disabled={actionLoading}>
                  <StopCircle className="w-4 h-4 mr-1" /> Stop
                </Button>
                <Button size="sm" variant="destructive" onClick={() => setDeleteTarget(selectedApp)} disabled={actionLoading}>
      ...
      <AlertDialog open={!!stopTarget} onOpenChange={(open) => !open && setStopTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will stop the application. Users will no longer be able to access it until you redeploy.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => stopTarget && handleStop(stopTarget)} disabled={actionLoading}>Stop</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete the application, its configuration, and volumes. This cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => deleteTarget && handleDelete(deleteTarget)} disabled={actionLoading} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
