...
import {
  ...
  HardDrive, Wifi, ChevronDown, ChevronUp, Server, GitBranch,
} from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage all your hosting services in one place</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md">
          <TabsTrigger value="traditional" className="flex items-center gap-2">
            <Server className="w-4 h-4" /> Web Hosting
          </TabsTrigger>
          <TabsTrigger value="github" className="flex items-center gap-2">
            <GitBranch className="w-4 h-4" /> App Deploy
          </TabsTrigger>
        </TabsList>

        {/* Traditional Hosting Tab */}
        <TabsContent value="traditional" className="mt-6">
          {/* Nameserver banner for pending_dns */}
          {accounts.some((a) => a.status === "pending_dns") && (
            <div className="bg-accent/5 border border-accent/20 rounded-lg p-4 flex items-start gap-3 mb-4">
          ...
          {loading ? (
            ...
            <div className="bg-card rounded-xl border border-border p-12 text-center">
              ...
              <h3 className="font-display font-semibold text-lg mb-2">No web hosting yet</h3>
              <p className="text-muted-foreground mb-4">Purchase a hosting plan for PHP sites, email, databases, and more.</p>
              <Link to="/dashboard/buy-hosting">
                  ...
                  >
                    <div className="p-5">
                      ...
                      {account.status === "active" && (
                        <Button variant="outline" size="sm" className="w-full" onClick={() => toggleExpand(account.id)}>
                      ...
                      <div className="border-t border-border p-5 bg-muted/30 space-y-5">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          ...
                          {plan.wordpress_enabled && (
                            <a href={account.wordpress_url || `https://${account.domain}/wp-admin`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2 p-4 rounded-lg bg-card border border-border hover:border-accent/30 hover:shadow-sm transition-all">
        ...
        {/* App Deploy Tab (Coolify-powered) */}
